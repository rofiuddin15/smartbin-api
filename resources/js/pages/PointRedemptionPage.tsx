            ...
            <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <select className="text-[12px] font-bold border-gray-200 rounded outline-none focus:border-admin-primary px-3 py-1 uppercase bg-white">
                                ...
                                return (
                                    <tr key={req.id} className="hover:bg-gray-50/50 transition-all group">
                                        ...
                                        <td className="px-6 py-4">
                                            <p className="text-sm font-black text-gray-800 uppercase tracking-tight">{req.user_name}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-base font-black text-gray-800">Rp {req.amount_idr.toLocaleString('id-ID')}</span>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {req.status === 'pending' ? (
                                                <div className="flex items-center justify-end gap-2">
                                                    <button className="bg-admin-success text-white px-4 py-1.5 rounded text-[11px] font-black uppercase tracking-widest hover:bg-green-700 transition-all shadow-md shadow-green-900/20">
                                                        Setujui
                                                    </button>
                                                    <button className="bg-white border border-red-100 text-red-600 px-4 py-1.5 rounded text-[11px] font-black uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">
                                                        Tolak
                                                    </button>
                                                </div>
                                            ) : (
                ...
                <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                    <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest">Total Pencairan Tertunda: <span className="font-black text-admin-primary">Rp 150.000</span></p>
                    <div className="flex gap-2">
                        <span className="text-[11px] font-black text-gray-300 uppercase">Halaman 1 dari 5</span>
                    </div>
                </div>
            </div>
        </div>
